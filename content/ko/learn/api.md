...
Flight은 쉽게 사용하고 이해할 수 있도록 설계되었습니다. 다음은 프레임워크를 위한 메소드의 전체 목록입니다. 이는 기본적인 정적 메소드인 코어 메소드와 필터링이나 재정의할 수 있는 매핑된 메소드인 확장 가능한 메소드로 구성되어 있습니다.

## 코어 메소드

이 메소드들은 프레임워크의 기본이며 재정의할 수 없습니다.
...
Flight::path(string $path) // 클래스를 자동으로 로드하기 위한 경로를 추가합니다.
...
Flight::getUrl(string $name, array $params = []) // 라우트 에일리어스를 기반으로 URL을 생성합니다.
...
`map`와 `register`로 추가된 사용자 정의 메소드는 필터링할 수도 있습니다.