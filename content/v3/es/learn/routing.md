...
Si quieres usar inyección de dependencias a través de un contenedor (PSR-11, PHP-DI, Dice, etc.), el
único tipo de rutas donde eso está disponible es creando el objeto directamente tú mismo
o usando cadenas para definir la clase y el método a llamar. Puedes ir a la página de [Dependency Injection](/learn/extending) 
...
// Consulta la página de Dependency Injection para más información sobre PSR-11
$dice = new \Dice\Dice();
...
De forma predeterminada, los patrones de ruta se coinciden contra todos los métodos de solicitud. Puedes responder
...
tu función de devolución de llamada. **Esto es más para la legibilidad de la ruta que para cualquier otra cosa. Por favor, consulta la sección a continuación sobre la advertencia importante.**
...
> **Nota:** Las coincidencias de grupos de regex `()` con parámetros posicionales no están soportadas. :'\(
...
Puedes especificar parámetros nombrados que son opcionales para la coincidencia envolviendo
...
La coincidencia solo se realiza en segmentos individuales de URL. Si quieres coincidir múltiples
...
Esto es especialmente útil si tu URL llega a cambiar. En el ejemplo anterior, digamos que users se movió a `/admin/users/@id` en su lugar.
Con el alias en su lugar, no tienes que cambiar en ninguna parte donde referencies el alias porque el alias ahora devolverá `/admin/users/5` como en el 
ejemplo anterior.

El alias de rutas aún funciona en grupos también:
...
Puedes usar la propiedad `executedRoute` o puedes solicitar que el objeto
de ruta se pase a tu devolución de llamada pasando `true` como el tercer parámetro en
...
> **Nota:** La propiedad `executedRoute` solo se establecerá después de que una ruta haya sido ejecutada. Si intentas acceder a ella antes de que una ruta haya sido ejecutada, será `NULL`. ¡También puedes usar executedRoute en middleware!

## Agrupación de Rutas

Puede haber ocasiones en que quieras agrupar rutas relacionadas (como `/api/v1`).
...
Consulta más detalles en la página de [group middleware](/learn/middleware#grouping-middleware).
...
Puedes configurar la `aliasBase`. De forma predeterminada, el alias es la última parte de la URL especificada.
Por ejemplo, `/users/` resultaría en un `aliasBase` de `users`. Cuando se crean estas rutas,
los alias son `users.index`, `users.create`, etc. Si quieres cambiar el alias, establece `aliasBase`
...
Estas son opciones de lista blanca y lista negra básicamente para que puedas especificar qué rutas quieres crear.
...
Puedes transmitir una respuesta al cliente usando el método `stream()` en una ruta. Si lo 
haces, debes establecer todos los métodos a mano antes de que salga nada al cliente.
...
Flight::route('/@filename', function($filename) {
	...
	// establece manualmente la longitud de contenido si quieres
...
Flight::route('/stream-users', function() {
	...
	// sin embargo, saques tus datos, solo como un ejemplo...
...
// Esta es cómo establecerás los encabezados antes de comenzar a transmitir.
	...
	// código de estado opcional, por defecto a 200
	'status' => 200
]);
```