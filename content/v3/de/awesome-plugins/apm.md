...
Willkommen bei FlightPHP APM – Ihrem persönlichen Leistungs-Coach für Ihre App! Diese Anleitung ist Ihre Roadmap, um Application Performance Monitoring (APM) mit FlightPHP einzurichten, zu verwenden und zu meistern. Ob Sie langsame Anfragen jagen oder sich einfach über Latenzdiagramme freuen möchten, wir haben Sie abgedeckt. Lassen Sie uns Ihre App schneller machen, Ihre Benutzer glücklicher und Ihre Debugging-Sitzungen zum Kinderspiel!

![FlightPHP APM](/images/apm.png)

## Warum APM wichtig ist

Stellen Sie sich vor: Ihre App ist ein volles Restaurant. Ohne eine Möglichkeit, zu verfolgen, wie lange Bestellungen dauern oder wo die Küche stockt, raten Sie nur, warum die Kunden verärgert abreisen. APM ist Ihr Sous-Chef – es beobachtet jeden Schritt, von eingehenden Anfragen bis zu Datenbankabfragen, und markiert alles, was Sie verlangsamt. Langsame Seiten verlieren Benutzer (Studien sagen, dass 53 % abbrechen, wenn eine Seite mehr als 3 Sekunden zum Laden braucht!), und APM hilft Ihnen, diese Probleme *bevor* sie schmerzen, zu erkennen. Es ist proaktiver Frieden – weniger „Warum ist das kaputt?“-Momente, mehr „Seht, wie reibungslos das läuft!“-Erfolge.

## Installation

Gehen Sie mit Composer los:
...
Sie benötigen:
- **PHP 7.4+**: Halten wir kompatibel mit LTS-Linux-Distros und unterstützen moderne PHP-Versionen.
...
**Warum zwei Orte?**
Unbearbeitete Metriken häufen sich schnell (denken Sie an unfiltrierte Protokolle). Der Worker verarbeitet sie in ein strukturiertes Ziel für das Dashboard. Das hält alles ordentlich!
...
- Stoppt, wenn erledigt oder keine Metriken mehr vorhanden sind.
...
```
- `--host 0.0.0.0`: Von jeder IP zugänglich (praktisch für Remote-Zugriff).
- `--port 8080`: Verwenden Sie einen anderen Port, wenn 8001 belegt ist.
- `--php-path`: Zeigen Sie auf PHP, wenn es nicht im PATH ist.
...
- **Umgekehrte Proxy**: Richten Sie Nginx oder Apache ein, um Anfragen an das Dashboard weiterzuleiten.
- **SSH-Tunnel**: Wenn Sie per SSH auf den Server zugreifen können, verwenden Sie `ssh -L 8080:localhost:8001 youruser@yourserver`, um das Dashboard auf Ihren lokalen Rechner zu tunneln.
- **VPN**: Wenn Ihr Server hinter einem VPN liegt, verbinden Sie sich damit und greifen Sie direkt auf das Dashboard zu.
- **Firewall konfigurieren**: Öffnen Sie Port 8001 für Ihre IP oder das Netzwerk des Servers (oder welchen Port Sie eingestellt haben).
- **Apache/Nginx konfigurieren**: Wenn Sie einen Webserver vor Ihrer Anwendung haben, können Sie ihn für eine Domain oder Subdomain konfigurieren. In diesem Fall setzen Sie das Dokumentenroot auf `/path/to/your/project/vendor/flightphp/apm/dashboard`.

#### Ein anderes Dashboard wünschen?
...
Das Dashboard ist Ihr APM-Hauptquartier – hier ist, was Sie sehen:
...
- **Langsamste Anfragen**: Die Top 5 Anfragen, die Zeit verbrauchen (z. B. „/api/heavy“ mit 2,5 Sekunden).
...
- **Latenz-Percentile**: 95. (p95) und 99. (p99) Antwortzeiten – kennen Sie Ihre worst-case-Szenarien.
...
**Was Sie erhalten**:
- Abfragtext (z. B. `SELECT * FROM users WHERE id = ?`)
- Ausführungszeit (z. B. 0,015 Sekunden)
- Zeilenanzahl (z. B. 42)

**Hinweis**:
- **Optional**: Überspringen Sie das, wenn Sie keine DB-Verfolgung benötigen.
- **Nur PdoWrapper**: Kern-PDO ist noch nicht integriert – bleiben Sie dran!
...
- `--daemon`: Läuft nonstop – ideal für Live-Überwachung.
...
Jede Anfrage hat eine eindeutige Request-ID zur Verfolgung. Sie können diese ID in Ihrer App verwenden, um Protokolle und Metriken zu korrelieren. Zum Beispiel können Sie die Request-ID auf einer Fehlerseite hinzufügen:
...
Beim Upgrade auf eine neuere Version von APM besteht die Möglichkeit, dass Datenbank-Migrationen ausgeführt werden müssen. Führen Sie dazu den folgenden Befehl aus:

```bash
php vendor/bin/runway apm:migrate
```
Das führt alle benötigten Migrationen aus, um das Datenbank-Schema auf die neueste Version zu aktualisieren.

**Hinweis:** Wenn Ihre APM-Datenbank groß ist, kann diese Migration einige Zeit in Anspruch nehmen. Führen Sie den Befehl möglicherweise in den Nebenzeiten aus.

## Bereinigen alter Daten

Um Ihre Datenbank aufzuräumen, können Sie alte Daten löschen. Das ist besonders nützlich, wenn Sie eine beschäftigte App betreiben und die Datenbankgröße handhabbar halten möchten.
Führen Sie dazu den folgenden Befehl aus:

```bash
php vendor/bin/runway apm:purge
```
Das entfernt alle Daten, die älter als 30 Tage sind. Sie können die Anzahl der Tage anpassen, indem Sie einen anderen Wert an die Option `--days` übergeben:

```bash
php vendor/bin/runway apm:purge --days 7
```
Das entfernt alle Daten, die älter als 7 Tage sind.

## Fehlerbehebung

Festsitzen? Probieren Sie diese:

- **Kein Dashboard-Daten?**
  - Läuft der Worker? Überprüfen Sie `ps aux | grep apm:worker`.
  - Stimmen die Konfigurationspfade? Verifizieren Sie, ob die DSNs in `.runway-config.json` auf echte Dateien zeigen.
...
- **Worker-Fehler?**
  ...
  - Überprüfen Sie die PHP-Protokolle auf Stack-Traces.
