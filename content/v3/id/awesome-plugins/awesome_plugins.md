...
Flight sangat mudah diperluas. Ada sejumlah plugin yang dapat digunakan untuk menambahkan fungsionalitas ke aplikasi Flight Anda. Beberapa didukung secara resmi oleh Tim Flight dan yang lainnya adalah pustaka mikro/lite untuk membantu Anda memulai.

## Alat AI

Flight dapat dibuat lebih keren dengan plugin berbasis AI.

- [Flight MCP](/awesome-plugins/mcp) - Plugin untuk mengintegrasikan MCP (Model Control Protocol) dengan Flight, memungkinkan fungsionalitas berbasis AI yang mulus. Terutama berfokus pada halaman dokumentasi, ini membantu menjaga biaya token tetap rendah dengan menyediakan informasi terbaru tentang proyek Flight Anda.
...
- [SwaggerUI](https://github.com/zircote/swagger-php) - Swagger UI adalah alat hebat untuk membantu menghasilkan dokumentasi API untuk proyek Flight Anda. Ini sangat mudah digunakan dan dapat disesuaikan sesuai kebutuhan Anda. Ini adalah pustaka PHP untuk membantu menghasilkan dokumentasi Swagger.

## Pemantauan Kinerja Aplikasi (APM)

Pemantauan Kinerja Aplikasi (APM) sangat penting untuk aplikasi apa pun. Ini membantu Anda memahami bagaimana aplikasi Anda berkinerja dan di mana titik penyumbatnya. Ada sejumlah alat APM yang dapat digunakan dengan Flight.
- <span class="badge bg-primary">resmi</span> [flightphp/apm](/awesome-plugins/apm) - Flight APM adalah pustaka APM sederhana yang dapat digunakan untuk memantau aplikasi Flight Anda. Ini dapat digunakan untuk memantau kinerja aplikasi Anda dan membantu mengidentifikasi titik penyumbat.

## Async

Flight sudah merupakan framework yang cepat, tetapi menambahkan mesin turbo padanya membuat segalanya lebih menyenangkan (dan menantang)!
...
- <span class="badge bg-primary">resmi</span> [flightphp/core SimplePdo](/learn/simple-pdo) - Helper PDO Flight resmi yang merupakan bagian dari inti. Ini adalah wrapper modern dengan metode helper yang nyaman seperti `insert()`, `update()`, `delete()`, dan `transaction()` untuk menyederhanakan operasi database. Semua hasil dikembalikan sebagai Collections untuk akses array/objek yang fleksibel. Bukan ORM, hanya cara yang lebih baik untuk bekerja dengan PDO.
...
- [knifelemon/easy-query](/awesome-plugins/easy-query) - Pembuat query SQL ringan dan fasih yang menghasilkan SQL dan parameter untuk pernyataan yang disiapkan. Bekerja dengan sangat baik dengan [SimplePdo](/learn/simple-pdo).
...
- <span class="badge bg-warning">deprecated</span> [flightphp/core View](/learn#views) - Ini adalah mesin templating sangat dasar yang merupakan bagian dari inti. Tidak disarankan untuk digunakan jika Anda memiliki lebih dari beberapa halaman di proyek Anda.
