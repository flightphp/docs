...
Runway — це CLI-застосунок, який допомагає керувати вашими Flight-застосунками. Він може генерувати контролери, відображати всі маршрути та багато іншого. Він базується на чудовій бібліотеці [adhocore/php-cli](https://github.com/adhocore/php-cli).
...
Вперше запустивши Runway, він спробує знайти конфігурацію `runway` у `app/config/config.php` через ключ `'runway'`.
...
Runway має низку команд, які ви можете використовувати для керування вашим Flight-застосунком. Є два простих способи використовувати Runway.
...
Дивіться [Документацію adhocore/php-cli](https://github.com/adhocore/php-cli) для отримання додаткової інформації про те, як створювати власні власні команди для вашого Flight-застосунку!
...
Якщо вам потрібно кастомізувати конфігурацію для Runway, ви можете встановити ці значення в `app/config/config.php`. Нижче наведено деякі додаткові конфігурації, які ви можете встановити:
...
Якщо вам потрібно ефективно доступатися до значень конфігурації, ви можете доступатися до них через метод `__construct` або метод `app()`. Також важливо зазначити, що якщо у вас є файл `app/config/services.php`, ці сервіси також будуть доступні для вашої команди.
...
Runway має деякі обгортки помічників, які полегшують для ШІ генерацію команд. Ви можете використовувати `addOption` та `addArgument` таким чином, що нагадує Symfony Console. Це корисно, якщо ви використовуєте інструменти ШІ для генерації ваших команд.
