...
Découvrez un [démonstration](https://flightphp-docs-apm.sky-9.com/apm/dashboard) du tableau de bord pour le site Flight Docs.
...
- **MySQL/MariaDB** : Idéale pour les projets plus grands ou les environnements de production où vous avez besoin d'un stockage robuste et scalable.
...
Cela maintient les performances fluides tout en vous fournissant des données solides.
...
- Par défaut, c'est SQLite — par exemple, `sqlite:/tmp/apm_metrics.sqlite` pour la source, un autre pour la destination.
...
> Ce processus vous demandera également si vous voulez exécuter les migrations pour cette configuration. Si c'est la première fois que vous la configurez, la réponse est oui.

**Pourquoi deux emplacements ?**
Les métriques brutes s'accumulent rapidement (pensez à des logs non filtrés). Le worker les traite en une destination structurée pour le tableau de bord. Cela garde les choses organisées !
...
- **Mode Daemon** :
  ...
  ```
  Tourne indéfiniment, traitant les métriques au fur et à mesure. Idéal pour le développement ou les petites configurations.
...
- **Tmux/Screen** :
  ...
  ```
  Le maintient en marche même si vous vous déconnectez.
...
- Affiche les logs de requêtes, les routes lentes, les taux d'erreurs, et plus encore.
...
- **Utiliser un Reverse Proxy** : Configurez Nginx ou Apache pour transférer les requêtes vers le tableau de bord.
...
- **Configurer Apache/Nginx** : Si vous avez un serveur web devant votre application, vous pouvez le configurer pour un domaine ou un sous-domaine. Si vous faites cela, vous définirez la racine des documents sur `/path/to/your/project/vendor/flightphp/apm/dashboard`
...
- **Journal des Requêtes** : Chaque requête avec horodatage, URL, code de réponse, et temps total. Cliquez sur « Détails » pour les middlewares, requêtes, et erreurs.
...
- **Taux d'Erreurs** : Pourcentage de requêtes qui échouent (par exemple, 2,3 % de 500).
...
- Activez le mode sombre pour ces sessions tardives.
...
**Où cela apparaît-il ?**
Dans les détails de requête du tableau de bord sous « Événements Personnalisés » — extensible avec un formatage JSON élégant.
...
- **PdoWrapper Seulement** : Le PDO de base n'est pas encore connecté — restez à l'écoute !
...
- `--daemon` : Tourne sans arrêt — idéal pour la surveillance en direct.
...
Flight::map('error', function($message) {
	// Récupérez l'ID de requête depuis l'en-tête de réponse X-Flight-Request-Id
...
**Note :** Si votre base de données APM est volumineuse, ces migrations peuvent prendre du temps. Vous pourriez vouloir exécuter cette commande pendant les heures creuses.
...
Pour garder votre base de données propre, vous pouvez purger les anciennes données. C'est particulièrement utile si vous exécutez une application occupée et voulez garder la taille de la base de données gérable.
...
- **Pas de Données dans le Tableau de Bord ?**
  ...
  - Les chemins de configuration correspondent-ils ? Vérifiez que les DSN de `.runway-config.json` pointent vers de vrais fichiers.
...
- **Le Tableau de Bord Ne Démarre Pas ?**
  - Port 8001 en cours d'utilisation ? Utilisez `--port 8080`.
...
- **Pas de Suivi des Requêtes de Base de Données ?**
  ...
  - Assurez-vous de définir le dernier argument dans le constructeur à `true`.